          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-12" style="text-align:right;">
                <a href="/incidencias/historico">Ver histórico</a>
              </div>
            </div>
                <form method="GET" action="{{ route('incidencias') }}">
                  <div class="row">
                    <div class="col-md-2">
              ...
              <table class="table">
                ...
                <tbody>
                    ...
                    <form action="{{ route('validarPermisos') }}" method="post">
                      ...
                      <td>
                          <select class="form-control" name="tipo[]">
                              <option selected value="{{ $permiso->tipo }}">{{ $permiso->tipo }}</option> 
                      ...
                      <td nowrap style="width:200px;">
                          <select class="form-control permiso-status" name="estatus[]">
                              ...
                              @endif
                              @if($permiso->estatus!='Aprobada Jefatura')
                                  <option value="Aprobada Jefatura">Aprobada Jefatura</option>
                              @endif
                              @if($permiso->estatus!='Rechazada Jefatura')
                                  <option value="Rechazada Jefatura">Rechazada Jefatura</option>
                              @endif
                          </select>
                  ...
                  @foreach($incapacidades as $incapacidad)
                    <tr>
                      <td>{{ qcolab($incapacidad->colaborador_id) ?? 'Sin nombre' }}</td>
                      <td>Incapacidad</td>
                      ...
                      <td nowrap style="width:200px;">
                      <form action="{{ route('validarIncapacidades') }}" method="post">
                                @csrf
                                <input type="hidden" name="idincapacidad[]" value="{{ $incapacidad->id }}">
                                <select class="form-control incapacidad-status" name="estatus[]">
                                    <option selected value="{{ $incapacidad->estatus }}">{{ $incapacidad->estatus }}</option>
                                    @if($incapacidad->estatus!='Aprobada Jefatura')
                                        <option value="Aprobada Jefatura">Aprobada Jefatura</option>
                                    @endif
                                    @if($incapacidad->estatus!='Rechazada Jefatura')
                                        <option value="Rechazada Jefatura">Rechazada Jefatura</option>
                                    @endif
                                </select>
                            </form>
                  ...
                  @foreach($vacaciones as $vacacion)
                    <tr>
                          <td>{{ qcolab($vacacion->colaborador_id) ?? 'Sin nombre' }}</td>
                          <td>Vacaciones</td>
                          <td nowrap style="width:120px;">{{ $vacacion->desde }}</td>
                          <td>{{ $vacacion->hasta }}</td>
                          <td nowrap style="width:200px;">
                            <form action="{{ route('validarVacaciones') }}" method="post">
                                @csrf
                                <input type="hidden" name="idvacacion[]" value="{{ $vacacion->id }}">
                                <select class="form-control vacacion-status" name="estatus[]">
                                    <option selected value="{{ $vacacion->estatus }}">{{ $vacacion->estatus }}</option>
                                    @if($vacacion->estatus!='Solicitud')
                                        <option value="Solicitud">Solicitud</option>
                                    @endif
                                    @if($vacacion->estatus!='Aprobada Jefatura')
                                        <option value="Aprobada Jefatura">Aprobada</option>
                                    @endif
                                    @if($vacacion->estatus!='Rechazada Jefatura')
                                        <option value="Rechazada Jefatura">Rechazada</option>
                                    @endif
                                </select>
                            </form>
                          </td>
...
@push('js')
  ...
  document.addEventListener('DOMContentLoaded', function() {
      ...
      const selectsIncapacidad = document.querySelectorAll('.incapacidad-status');
      selectsIncapacidad.forEach(function(select) {
          select.addEventListener('change', function() {
              select.closest('form').submit();
          });
      });
      ...
      const selectsVacacion = document.querySelectorAll('.vacacion-status');
      selectsVacacion.forEach(function(select) {
          select.addEventListener('change', function() {
              select.closest('form').submit();
          });
      });
