@extends('layouts.app', ['activePage' => 'Mensajes', 'menuParent' => 'laravel', 'titlePage' => __('Editar Mensaje')])
...
<div class="content">
    <form action="{{ route('mensajes.update', $mensaje) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="grupo_id" id="grupo_id" class="form-control" required>
                ...
                @foreach ($grupos as $grupo)
                    <option value="{{ $grupo->id }}" {{ $grupo->id == $mensaje->grupo_id ? 'selected' : '' }}>
                        {{ $grupo->nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <textarea id="mensaje" name="mensaje" class="form-control" rows="10">{{ $mensaje->contenido }}</textarea>
        </div>

        <div class="form-group">
            <label for="fecha_inicio">Fecha de Inicio</label>
            <input type="date" name="fecha_inicio" id="fecha_inicio" class="form-control" value="{{ $mensaje->fecha_inicio }}" required>
        </div>

        <div class="form-group">
            <label for="fecha_fin">Fecha de Fin</label>
            <input type="date" name="fecha_fin" id="fecha_fin" class="form-control" value="{{ $mensaje->fecha_fin }}">
        </div>

        <button type="submit" class="btn btn-info btn-sm">Actualizar Mensaje</button>
    </form>
</div>
